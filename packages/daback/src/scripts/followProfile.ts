...
async function main() {
  ...
  console.log(`Free follow of ${recommendedProfiles[0].id} triggered with proxyActionId: `, proxyActionResult.unwrap());
  ...
  const followBroadcastResultValue = await signAndBroadcast(lensClient, wallet, followTypedDataResult);
  ...
  console.log(`Transaction to follow ${recommendedProfiles[1].id} was successfuly broadcasted with txId ${followBroadcastResultValue.txId}`);
  ...
  const unfollowBroadcastResultValue = await signAndBroadcast(lensClient, wallet, unfollowTypedDataResult);
  ...
  console.log(`Transaction to unfollow ${recommendedProfiles[1].id} was successfuly broadcasted with txId ${unfollowBroadcastResultValue.txId}`);
}
