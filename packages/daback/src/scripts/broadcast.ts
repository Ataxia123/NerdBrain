...
async function main() {
  ...
  const signedTypedData = await wallet._signTypedData(data.typedData.domain, data.typedData.types, data.typedData.value);
  ...
  console.log(`Transaction to set dispatcher for profile ${activeProfile.id} was successfuly broadcasted with txId ${broadcastResultValue.txId}`);
