...
export default function LensComponent() {
  ...
  async function createPost() {
    ...
    const { encryptedMetadata, contentURI } = await uploadToIPFS();
  ...
  return (
    <div>
      ...
      {!address && (
        <button className={baseButtonStyle} onClick={connect}>
          Connect
        </button>
      )}
      ...
      {address && token && (
        <div className={containerStyle}>
          <div>
            <button className={buttonStyle(gatingType, "nft")} onClick={() => setGatingType("nft")}>
              Gate with NFT
            </button>
            <button className={buttonStyle(gatingType, "erc20")} onClick={() => setGatingType("erc20")}>
              Gate with ERC20 token
            </button>
          </div>
          {gatingType == "nft" && (
          ...
          <textarea onChange={onChange} className={textAreaStyle} placeholder="Encrypted post content" />
          <button className={submitButtonStyle} onClick={createPost}>
            Submit
          </button>
        </div>
      )}
